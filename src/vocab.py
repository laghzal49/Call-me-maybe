...
class Vocab:
    """Wraps the model's tokenizer: encode/decode (with caching) plus the
    token-id groups derived from the vocab file, used to mask logits during
    ...
    def encode(self, text: str) -> List[int]:
        ...
        Fixed JSON literals (braces, commas, key names, function names) repeat
        identically on every prompt, so results are cached to avoid redundant
        tokenizer calls (bonus: performance optimization).
        """
