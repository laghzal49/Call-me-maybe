...
class Small_LLM_Model:
    ...
    def get_logits_from_input_ids(self, input_ids: list[int]) -> list[float]:
        ...
        common_len = 0
        for i in range(min(len(input_ids), len(self._cached_ids))):
            if input_ids[i] == self._cached_ids[i]:
                common_len += 1
            else:
                break

        if common_len == len(self._cached_ids) and self._cached_pkv is not None:
            new_ids = input_ids[common_len:]
            past_key_values = self._cached_pkv
            if not new_ids:
        ...
        self._cached_pkv = out.past_key_values
        self._cached_logits = [float(x) for x in out.logits[0, -1].tolist()]
